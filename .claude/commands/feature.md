...
3. **Pattern Analysis (REQUIRED)**
   ...
   Before finalizing the plan, you MUST analyze existing patterns to prevent duplicate implementations:

   - Search for existing methods, classes, and utilities with similar names or functionality
   - Use Grep to find methods that might already do what you need
   - Document methods that MUST be reused instead of recreated
   - If proposing a new method, verify no equivalent exists in the codebase
   - Check for cross-project implementations in `specs/implementations/` directory
   - Fill out the `## Existing Patterns to Follow` section in the plan

   **Anti-pattern prevention:**
   - Do NOT create new methods if equivalent ones exist
   - Do NOT duplicate utility functions across files
   - Do NOT introduce new patterns when established ones exist
...
### Methods to Reuse (DO NOT recreate)

<list existing methods that should be used instead of creating new ones>
...
<describe existing architectural patterns, coding conventions, or approaches this feature must follow>

### Anti-Patterns (DO NOT do)

<list specific things to avoid, such as creating duplicate methods or diverging from established patterns>
